...
export class JankLadder extends LoggedManager {
    ...
    constructor(client) {
        ...
        this.db.prepare("CREATE TABLE IF NOT EXISTS jankladder (id INTEGER PRIMARY KEY, time TEXT, architype TEXT, mode TEXT, raceRoom TEXT , raceActive BOOL)").run();
        this.initSchedule();
        this.refreshSchedule();
    }

    addRaceToSchedule(time: Date, architype: string, mode: string): number | bigint {
        const insert = this.db.prepare("INSERT INTO jankladder (id, time, architype, mode, raceRoom, raceActive) VALUES (?, ?, ?, ?, ?, ?)");
        let result = insert.run(null, time.toISOString(), architype, mode, null, 0);
        this.fetchSchedule();
        return result.lastInsertRowid;
    }

    updateRaceInSchedule(id: number, time: Date, architype: string, mode: string, raceRoom: string|null, raceActive: number): void {
        const update = this.db.prepare("UPDATE jankladder SET time = ?, architype = ?, mode = ? , raceRoom = ? , raceActive = ? WHERE id = ?");
        update.run(time.toISOString(), architype, mode, raceRoom, raceActive, id);
        this.fetchSchedule();
    }

    fetchSchedule() {
        //this.logger.debug(`Fetching schedule...`, this);
        let scheduleRaw = this.db.prepare(`SELECT * FROM jankladder`);
    ...
    async raceWatcher() {
        try {
            ...
            for (const entry of upcomingRaces) {
                entry.time.setHours(entry.time.getHours() - 1);
                //If 30 minutes before race, create a race room for it, unless it is more than 30 minutes old
                if (entry.time.getTime() - now.getTime() < 1000 * 60 * Config.jankladder.prestart && entry.time.getTime() > cutoffTime.getTime()) {
                    if (!entry.raceRoom) {
                        //
                        ...
                        if (raceRoom) {
                            ...
                            this.updateRaceInSchedule(entry.id, entry.time, entry.architype, entry.mode, raceRoom.url, 0);
                        }
                            //*/
                    } else {
                        ...
                        if(entry.time.getTime() - now.getTime() < 1000 * 15 && entry.raceActive === 0) {
                            this.client.logger.info(`Race starting in less 15 seconds, starting it`, this);
                            this.racetime.startRace(entry.raceRoom.url);
                            this.updateRaceInSchedule(entry.id, entry.time, entry.architype, entry.mode, entry.raceRoom.url, 1);
                        }
                    }
                }
            }
